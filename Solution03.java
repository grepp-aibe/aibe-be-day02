...
public class Solution03 {
    public static void main(String[] args) {
        ...
        // - 객체는 어딘가에 담겨있고, 그 담긴 주소가 기본적으론 출력 시 기본값.
        // 문자열은 하도 너무 사람들이 출력하다보니까 (그 값 자체를) 그냥 그래 넌, 그 텍스트 묶음을 연결한 걸 출력해 라고 정의해버린것.
        // toString -> 오버라이딩. 모든 객체는!!!! 주소가 찍혀 나온다!!!
        // 어? 그래요? 그러면 노란색인 걸보니 답은? 눌러보면 나오지 않나요?
        // deep하게 가자
        System.out.println("Arrays.deepToString(arr) = " + Arrays.deepToString(arr));

        int arr2[][] = {{1, 2}}; // 이런 역할이에요 아시겠어요? -> 어디서 나온 영향?

        System.out.println("arr2 = " + arr2); // 주소값 + alt + enter
        ...
        // 3번 향상된 for문
        String[] fruits = {"apple", "pear", "grapes"}; // 이것도 겨우겨우 어떻게함. {}는 어케했네? 다행이다... []
        for (int i = 0; i < fruits.length; i++) {
//        for (let i = 0; i < fruits.length; i++) { // ????? 이러고서 뭐 잘못됐는지 못찾음. 밑줄을 보세요 제발
            System.out.println(fruits[i]);
        }
        // iter
        for (String fruit : fruits) { // for (const v of arr) {}
//        for (String fruit of fruits) { // 이러지마 제발...
//        for (const fruit : fruits) { // 이러지마 제발... 2
            // console.log(fruit); // ㅠㅠ 아아아아 제발...
